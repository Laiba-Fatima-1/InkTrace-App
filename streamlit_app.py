...
with tab1:
    ...
    <div style="display:flex; gap:20px; align-items:flex-start; flex-wrap:wrap;">
        <canvas id="drawCanvas" width="280" height="280"
            style="border:2px solid #7A2E3B; border-radius:8px; background:#262138; touch-action:none; cursor:crosshair;">
        </canvas>
        <div style="display:flex; flex-direction:column; gap:10px;">
            <button id="clearBtn" style="padding:10px 20px; border-radius:8px; border:none;
                background:#3F6C51; color:white; font-family:Inter,sans-serif; font-weight:600; cursor:pointer;">
                Clear
            </button>
            <button id="submitBtn" style="padding:10px 20px; border-radius:8px; border:none;
                background:#7A2E3B; color:white; font-family:Inter,sans-serif; font-weight:600; cursor:pointer;">
                Read My Handwriting
            </button>
        </div>
    </div>
    <script>
        ...
        document.getElementById('submitBtn').addEventListener('click', function() {
            ...
            window.top.location.href = topUrl + '?img=' + encodeURIComponent(base64) + '&t=' + Date.now();
        });
    </script>
    """
    ...
    if "img" in query_params:
        try:
            import base64
            img_data = base64.b64decode(query_params["img"])
            ...
            st.warning(f"Couldn't read that drawing — try again. ({e})")
